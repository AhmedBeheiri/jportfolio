...
# Define command to run the script and then the server with migrations applied
CMD ["/bin/bash", "-c", "./deploy/docker-entrypoint.sh ./server --mode=production --server-id=default --logging=normal --role=monolith --apply-migrations"]
